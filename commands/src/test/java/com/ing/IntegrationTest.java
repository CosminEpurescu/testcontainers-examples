...
public class IntegrationTest {
      ...
      (MySQLContainer)
          new MySQLContainer(DockerImageName.parse(MYSQL_IMAGE))
              ...
              .withCopyFileToContainer(
                  MountableFile.forClasspathResource("init_db.sql", 0777), "/init_db.sql")
              .withCopyFileToContainer(
                  MountableFile.forClasspathResource("init_db.sh", 0777), "/init_db.sh")
              .withCopyFileToContainer(
                  MountableFile.forClasspathResource("clear_db.sql", 0777), "/clear_db.sql")
              .withCopyFileToContainer(
                  MountableFile.forClasspathResource("clear_db.sh", 0777), "/clear_db.sh");
  ...
  public void beforeEach() throws IOException, InterruptedException {
    // init the database
    mySQLContainer.execInContainer("sh", "init_db.sh");
  }

  @AfterEach
  public void afterEach() throws IOException, InterruptedException {
    // clear the database
    mySQLContainer.execInContainer("sh", "clear_db.sh");
  }
